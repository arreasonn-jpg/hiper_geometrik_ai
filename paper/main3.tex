...
\emph{implicit regulariser}. Across five noise levels and 20 seeds,
...
Kronecker wins only $1/20$ seeds. The bottleneck is optimisation,
...
($Y = AXB$) Flat Kronecker wins only $1/20$ seeds. The bottleneck
...
(depending on task), 20 seeds per configuration.
...
$r = +0.953$, $p = 0.012$ (n=5). \textbf{H2 is confirmed.}
...
\begin{table}[h]
\centering
\caption{Non-grid control tasks (20 seeds). Kronecker loses on
...
Kronecker loses by $21\%$ on a graph-structured task. The
...
Flat Kronecker wins only $1/20$ seeds. This isolates an
...
structure, which explains the $18/20$ win rate. This is
