...
\begin{abstract}
Hibrit sistemlerin kazancı için üç bağımsız boyutta birleşik bir teori
sunuyoruz: (1) metrik tipi---lineer (doğruluk, bit-doğruluğu,
token-doğruluğu) ve lineer olmayan ($F_1$); (2) mimari tipi---bilinear
Kronecker ve yoğun çok-katmanlı algılayıcı; (3) görev yapısı---rastgele
lineer ve kompozisyonel (grid uyumlu). Her boyut için \emph{hangi}
yapının \emph{ne zaman} kazandığını 20--50 bağımsız tohumdan
istatistiksel kanıtla belirliyoruz. Önceki çalışmamızdaki lineer metrik
formülünü, makro $F_1$ için kesin olan sınıf-başına ağırlıklı bir
özdeşliğe genişletiyoruz (20 NER konfigürasyonunda makine hassasiyetinde
hata), ve Kronecker mimarisinin basit kompozisyonel görevlerde $8\times$
daha parametre-verimli, görüntü-benzeri kompozisyonel görevlerde ise
istatistiksel olarak üstün olduğunu gösteriyoruz (50 tohum,
$p<0.0001$, Cohen $d=-1.37$). Hibrit sistem tasarımının ilkeli
olabileceği sonucuna varıyoruz: metrikler için formül-güdümlü,
görevler için yapı-güdümlü.
\end{abstract}
...
\paragraph{Görev-Uygunluk Hipotezi.}
Kronecker mimarileri, görevin fonksiyonel bağımlılıkları $X \mapsto A X B$
tensör faktörizasyonuna uyduğunda parametre-verimlidir. Tersine, hedefi
genel (faktörize edilemez) bir lineer dönüşüm olan görevlerde aynı kısıt
bir darboğaza dönüşür.

Bu hipotezi Bölüm~\ref{sec:experiments}'te üç görev tipinde test ediyoruz.
