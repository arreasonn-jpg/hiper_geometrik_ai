...
$8\times$--$12.6\times$ more parameter-efficient on grid-structured,
noisy tasks; the advantage disappears on graph tasks and on exact
bilinear tasks (optimisation-limited), and reverses under clean data.
Kronecker's benefit is therefore conditional on both structural
alignment and regularisation need. These results suggest that hybrid system
...
Combining symbolic reasoning with neural learning is a long-standing
goal. Garcez and Lamb~\cite{garcez2023neurosymbolic} survey the
third wave of neurosymbolic AI, and DeepProbLog
\cite{manhaeve2018deepproblog} integrates probabilistic logic
programming with neural networks. Besold et al.~\cite{besold2017neural}
provide an earlier overview. Our work differs in \emph{when} the
combination wins: we provide a closed-form gain formula rather than
a new integration architecture.

\paragraph{Hybrid systems with compositional structure.}
Neural Module Networks \cite{andreas2016neural} and the CLEVR
benchmark \cite{johnson2017clevr} demonstrated that compositional
tasks can be decomposed into reusable modules. Mao et
al.~\cite{mao2019neuro} extended this to concept learning. Our
compositional tasks are inspired by these but focus on a different
question: given a hybrid system, when does its symbolic arm help?

\paragraph{Tensorized and structured architectures.}
MLP-Mixer \cite{tolstikhin2021mlp} and gMLP \cite{liu2021gmlp}
replace attention with token-mixing MLPs, while ConvMixer
\cite{trockman2023patches} shows that patch-based convolutions achieve
comparable performance. Tensor Train decomposition
\cite{novikov2015tensor} and LoRA \cite{hu2021lora} exploit low-rank
structure for efficiency. Our Kronecker layer is closely related to
these approaches, but we study \emph{task-appropriateness} rather
than proposing a new layer.

\paragraph{Attention and vision transformers.}
The Transformer \cite{vaswani2017attention} and its vision variant ViT
\cite{dosovitskiy2020vit} established attention as a general-purpose
operator. Our work does not compete with attention; instead, we
characterise when a \emph{structured} alternative (Kronecker) is
preferable to a \emph{unstructured} baseline (dense MLP).

\paragraph{Cross-lingual NLP.}
Universal Dependencies \cite{nivre2020ud} provides the multilingual
treebanks used in our validation. The ITU Turkish NLP Pipeline
\cite{eryigit2015itu} is a canonical example of a rule-based symbolic
system for a morphologically rich language, and motivated our choice
of Turkish as one of the twelve validation languages.

\paragraph{Metrics and their decomposition.}
Sokolova and Lapalme~\cite{sokolova2009systematic} provide a
systematic analysis of performance measures, and Opitz and
Burst~\cite{opitz2019macro} address macro $F_1$ specifically.
Grandini et al.~\cite{grandini2020metrics} compare multi-class metrics
in depth. Our per-class identity is, to our knowledge, the first
exact decomposition of hybrid $F_1$ in terms of per-class weights.

\paragraph{Statistical methodology.}
We follow standard practice for multiple-comparison correction
\cite{benjamini1995controlling} and effect-size reporting
...
\paragraph{Working Hypothesis (later revised).}
We initially proposed that Kronecker architectures are parameter-efficient
when the task's functional dependencies respect the tensor factorisation
$X \mapsto A X B$. Section~\ref{sec:regularization} shows that this
\emph{structural alignment} hypothesis is rejected, and that the actual
mechanism is implicit regularisation via the parameter constraint.
...
Source: \texttt{artifacts/TWELVE\_LANGUAGE\_FORMULA.md},
\texttt{artifacts/SYMBOLIC\_NOISE\_ROBUSTNESS.md}.
Script: \texttt{artifacts/symbolic\_noise\_robustness.py}.
...
\paragraph{Reproducibility.}
Source: \texttt{artifacts/F1\_HARMONIC\_FORMULA\_FINAL.md}.
Script: \texttt{artifacts/f1\_perclass\_test.py} (seed 1--5, hidden ratios
$\{0.10, 0.30, 0.50, 0.70\}$).
...
\paragraph{Reproducibility.}
Sources: \texttt{artifacts/COMPOSITIONAL\_KRONECKER\_WIN.md},
\texttt{artifacts/IMAGE\_COMPOSITIONAL\_WIN.md}.
Scripts: \texttt{artifacts/compositional\_20seed.py} (seeds 1--20),
\texttt{artifacts/image\_compositional.py} (seeds 1--50).
...
\paragraph{Reproducibility.}
Source: \texttt{artifacts/MNIST\_KRONECKER.md}.
Script: \texttt{artifacts/mnist\_kronecker.py} (seeds 1--5,
MNIST loaded from \texttt{data/mnist/}).

\subsection{Parameter-Matched Comparison}
\label{sec:param-matched}

The $8\times$ efficiency claim compares Kronecker and Dense MLP
\emph{at their natural parameter counts}. To test whether the
efficiency is robust under matching, we deepened Flat Kronecker
until its parameter count exceeded that of Dense MLP.

\begin{table}[h]
\centering
\caption{Parameter-matched comparison on simple compositional task.
Deep Flat Kronecker collapses even with more parameters.}
...
Model & Parameters & Test Accuracy \\
\midrule
Dense MLP ($h=128$)          & $20{,}868$ & $0.798$ \\
Flat Kronecker ($K=3$)       & $2{,}564$  & $0.801$ \\
Flat Kronecker ($K=16$)      & $4{,}356$  & $0.402$ \\
Flat Kronecker ($K=168$)     & $21{,}764$ & $0.257$ \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Finding.}
Matching Dense MLP's parameter count requires $K=168$ layers, at
which point Flat Kronecker's accuracy drops to $0.257$ --- worse
than random ($1/4 = 0.25$). The collapse is structural: repeated
bilinear + SiLU without normalisation destroys information. The
$8\times$ efficiency claim is therefore a statement about the
\emph{useful regime} ($K \leq 3$), not a universal property.

\subsection{Non-Grid Tasks (Circular Logic Control)}
\label{sec:non-grid}

A potential weakness of the previous sections is circular design: if
the tasks are grid-structured, and Kronecker excels on grids, the
advantage may be an artifact of task construction. We therefore tested
four tasks outside the grid regime.

\begin{table}[h]
\centering
\caption{Non-grid control tasks (20 seeds). Kronecker loses on graph
and exact-bilinear tasks.}
\label{tab:non-grid}
\small
\begin{tabular}{lccc}
\toprule
Task & Dense MLP & Flat Kronecker & Flat Wins \\
\midrule
Grid baseline        & $0.819$ & $0.822$ & $11/20$ \\
Sequence (1D)        & $0.745$ & $\mathbf{0.780}$ & $18/20$ \\
Graph (no grid)      & $\mathbf{0.821}$ & $0.609$ & $0/20$ \\
Random factor ($Y = AXB$) & $\mathbf{0.706}$ & $0.599$ & $1/20$ \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Findings.}
(1)~\emph{Grid required:} On the graph task, Kronecker loses by
$21\%$; the advantage does not transfer to non-grid structure.
(2)~\emph{Optimisation, not expressivity:} The random-factor task is
\emph{exactly} a bilinear form $Y = AXB$, which a Kronecker layer
represents natively. Yet Flat Kronecker wins only $1/20$ seeds.
The limitation is optimisation, not representational capacity.
(3)~\emph{Sequence works via reshape:} A 1D sequence reshaped to
$16 \times 16$ gains artificial 2D structure, which explains the
$18/20$ win rate.

\paragraph{Combined hypothesis.}
Kronecker architectures require \emph{both} conditions:
(i) grid/tensor structure in the input, and (ii) a regularisation
need from noisy data. Neither alone suffices.

\subsection{Implicit Regularization Hypothesis}
\label{sec:regularization}

The task-appropriateness hypothesis (Section~\ref{sec:kronecker}) was
formalised as ``Kronecker architectures are efficient when the task's
structure aligns with tensor factorisation.'' We tested this on six
tasks (random linear, permuted MNIST, noise MNIST, block compositional,
grid correlated, Fashion-MNIST) and found the prediction
\emph{reversed}: autocorrelation correlates \emph{negatively} with
Kronecker's win rate ($r = -0.38$, $p = 0.46$).

A subsequent ablation revealed a different mechanism: \emph{noise
level} predicts Kronecker's advantage ($r = +0.95$, $p = 0.012$).
Table~\ref{tab:noise-ablation} summarises.

\begin{table}[h]
\centering
\caption{Noise ablation on block compositional task (20 seeds).
Kronecker's parameter constraint acts as implicit regularization.}
\label{tab:noise-ablation}
\small
\begin{tabular}{cccc}
\toprule
Noise & Dense MLP & Flat Kronecker & Flat Wins \\
\midrule
...
\paragraph{Revised hypothesis.}
Kronecker's $2n^2$-parameter constraint functions as an architectural
regulariser. On clean data it limits expressivity and loses to Dense
MLP; on noisy data it prevents overfitting and wins. The transition
occurs at noise $\approx 0.30$.

This reformulation subsumes the earlier observation and provides a
\emph{mechanistic} explanation: the gain is not structural alignment
but implicit regularisation.
...
\texttt{github.com/arreasonn-jpg/hiper\_geometrik\_ai}. Seeds and
hyperparameters are fixed in the scripts listed above. The MNIST
dataset is automatically downloaded from the standard Google Cloud
mirror. No pretrained models are used.
...
T.~R.~Besold et al. Neural-symbolic learning and reasoning: A survey
...
J.~Johnson et al. CLEVR: A diagnostic dataset for compositional
language and elementary visual reasoning. \emph{CVPR}, 2017.

\bibitem{mao2019neuro}
J.~Mao et al. The neuro-symbolic concept learner: Interpreting
scenes, words, and sentences from natural supervision. \emph{ICLR}, 2019.

\bibitem{tolstikhin2021mlp}
I.~Tolstikhin et al. MLP-Mixer: An all-MLP architecture for vision.
\emph{NeurIPS}, 2021.

\bibitem{liu2021gmlp}
H.~Liu et al. Pay attention to MLPs. \emph{NeurIPS}, 2021.

\bibitem{trockman2023patches}
A.~Trockman and J.~Z.~Kolter. Patches are all you need?
\emph{TMLR}, 2023.

\bibitem{novikov2015tensor}
A.~Novikov, D.~Podoprikhin, A.~Osokin, and D.~Vetrov. Tensorizing
...
J.~Opitz and S.~Burst. Macro F1 and macro F1. \emph{arXiv preprint
